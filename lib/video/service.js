    ...
    scenario: {
      ...
      scenes: scenario.scenes.map((scene, index) => {
        ...
        return {
          ...
          voiceover: optionalInputText(scene.voiceover, 800),
...
function normalizeCreatedProviderJob(job) {
  const providerJobId = optionalInputText(job?.providerJobId, 500);
...
function normalizeProviderUpdate(update, fallbackProgress) {
  if (!update || typeof update !== 'object' || !VIDEO_JOB_STATES.has(update.state)) {
    throw new AppError(
      502,
      'VIDEO_PROVIDER_INVALID_RESPONSE',
  ...
  return {
    ...
    error: update.state === 'failed'
      ? optionalInputText(update.error, 1000) || undefined
      : undefined
  };
}

function requiredPlaybackUrl(value) {
  const playbackUrl = optionalInputText(value, 2000);
