...
import { AppError, assertIqWikiUrl, isPublicHttpsUrl } from '../foundation.js';
...
export class VideoJobService {
  ...
  async createJob(input) {
    ...
    try {
      const providerJob = normalizeCreatedProviderJob(
        await callVideoProvider(() => this.provider.create(normalized), this.config.providerTimeoutMs)
      );
  ...
  async getJob(id, refresh = true) {
    ...
    if (refresh && ['queued', 'processing'].includes(job.state)) {
      const update = normalizeProviderUpdate(
        await callVideoProvider(() => this.provider.poll(job.providerJobId), this.config.providerTimeoutMs),
        job.progress,
        job.state
      );
      Object.assign(job, {
  ...
  async retryJob(id) {
    ...
    this.ledger.reserve(this.config.maxJobUsd, this.config.dailyCapUsd);
    try {
      const next = normalizeCreatedProviderJob(
        await callVideoProvider(() => this.provider.create(job.input), this.config.providerTimeoutMs)
      );
      Object.assign(job, {
  ...
  async cancelJob(id) {
    ...
    await callVideoProvider(() => this.provider.cancel(job.providerJobId), this.config.providerTimeoutMs);
    job.state = 'cancelled';
  ...
  requireJob(id) {
    if (
      ...
      throw new AppError(400, 'INVALID_VIDEO_JOB_ID', 'Video job ID is invalid.');
    }
...
async function callVideoProvider(operation, timeoutMs = 30000) {
  let timeout;
  try {
    return await Promise.race([
      Promise.resolve().then(operation),
      new Promise((_, reject) => {
        timeout = setTimeout(() => reject(new Error('Video provider timed out.')), timeoutMs);
      })
    ]);
  } catch {
    throw new AppError(
...
function normalizeProviderUpdate(update, fallbackProgress, currentState) {
  ...
  return {
    ...
    progress,
    playbackUrl,
    error: update.state === 'failed' ? 'Video generation failed.' : undefined
  };
}
...
function requiredPlaybackUrl(value) {
  ...
  try {
    if (!isPublicHttpsUrl(playbackUrl)) throw new Error();
