...
function milliseconds(value, name, fallback) {
  ...
  if (!Number.isSafeInteger(amount) || amount < 1000 || amount > 120000) {
    throw new AppError(503, 'VIDEO_CONFIGURATION_ERROR', `${name} must be between 1000 and 120000.`);
  }
  return amount;
}
...
export function getVideoConfig(env = process.env) {
  ...
  return {
    ...
    maxJobUsd,
    dailyCapUsd,
    providerTimeoutMs: milliseconds(env.VIDEO_PROVIDER_TIMEOUT_MS, 'VIDEO_PROVIDER_TIMEOUT_MS', 30000),
